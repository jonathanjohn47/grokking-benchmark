...
4. ⏳ **AGE (Adaptive Grokking Epoch)** — next
5. ⏳ HTSR Alpha
6. ⏳ Correlation Traps
7. ⏳ Weight-PCA
8. ⏳ Higher-MI
...
September 6, 2026 — Spectral predictor (Canatar et al. 2021 task-model alignment) formally closed, negative: 5-seed checkpoint-only recompute on the Nanda-Unified four-head model shows `k_90_min` and `alignment_max` landing at or after grok in 5/5 seeds; the old per-weight-matrix SVD (`stable_rank`) proxy and its results were deleted first. Three of nine predictors now closed (L2 Norm, Dropout, Spectral — all negative); AGE is next. (Previous update: September 4, 2026 — Dropout predictor reopened after a literature match with Salah & Yevick, arXiv:2507.11645; subsequently implemented and closed negative on September 5, 2026.)
